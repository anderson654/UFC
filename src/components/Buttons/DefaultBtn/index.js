...
export default function DefaultBtn({ style, backgroundColor, color, text = '', onPress = () => { }, textTransform = 'uppercase' }) {
    ...
    return (
        <TouchableRipple onPress={onPress} style={{ flexDirection: "row", justifyContent: "center", alignItems: "center", backgroundColor: backgroundColor || "#fff", borderRadius: 3, marginBottom: 20, ...style }}>
            <Text style={[textCardResults, { textTransform: textTransform, padding: 14, fontSize: 16, color: color || '#000' }]}>{text}</Text>
        </TouchableRipple>
    )
}